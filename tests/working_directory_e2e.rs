...
async fn test_active_session_working_directory() {
    ...
    let get_response1 = client
        .get(format!("{}/api/v1/sessions/active-session-1", server.base_url))
        .send()
        .await
        .unwrap();
    ...
    debug!("Verified first session working directory: {:?}", session1.working_directory);
    
    // Test GET session for second session
    let get_response2 = client
        .get(format!("{}/api/v1/sessions/active-session-2", server.base_url))
        .send()
        .await
        .unwrap();
    ...
    debug!("Verified second session working directory: {:?}", session2.working_directory);
...
async fn test_inactive_session_working_directory_from_disk() {
    ...
    let get_response1 = client
        .get(format!("{}/api/v1/sessions/disk-session-1", server.base_url))
        .send()
        .await
        .unwrap();
    ...
    debug!("Verified first disk session working directory: {:?}", session1.working_directory);
    
    // Test GET session for second disk session
    let get_response2 = client
        .get(format!("{}/api/v1/sessions/disk-session-2", server.base_url))
        .send()
        .await
        .unwrap();
    ...
    debug!("Verified second disk session working directory: {:?}", session2.working_directory);
    
    // Test GET session for third disk session
    let get_response3 = client
        .get(format!("{}/api/v1/sessions/disk-session-3", server.base_url))
        .send()
        .await
        .unwrap();
    ...
    debug!("Verified third disk session working directory: {:?}", session3.working_directory);
...
async fn test_session_resume_working_directory() {
    ...
    let get_response = client
        .get(format!("{}/api/v1/sessions/{}", server.base_url, &resume_body.session_id))
        .send()
        .await
        .unwrap();
    ...
    info!("Successfully verified resumed session uses correct working directory: {:?}", session.working_directory);
}
    ...
    for session_info in &list_body.sessions {
        match session_info.session_id.as_str() {
            "active-list-session" => {
                ...
                debug!("Verified active session in list: {:?}", session_info.working_directory);
            }
            "inactive-list-1" => {
                assert_eq!(session_info.working_directory, PathBuf::from("/home/user/list1"));
                assert!(!session_info.active);
                debug!("Verified inactive session 1 in list: {:?}", session_info.working_directory);
            }
            "inactive-list-2" => {
                assert_eq!(session_info.working_directory, PathBuf::from("/home/user/list2"));
                assert!(!session_info.active);
                debug!("Verified inactive session 2 in list: {:?}", session_info.working_directory);
            }
...
async fn test_mixed_active_inactive_working_directories() {
    ...
    for (session_id, expected_dir, should_be_active) in test_cases {
        ...
        debug!("Verified session {} working directory: {:?} (active: {})", 
               session_id, session.working_directory, should_be_active);
    }
    
    info!("Successfully verified mixed active/inactive sessions with different working directories");
}
...
async fn test_working_directory_session_not_found() {
    ...
    let get_response = client
        .get(format!("{}/api/v1/sessions/non-existent-session", server.base_url))
        .send()
        .await
        .unwrap();
...
async fn test_working_directory_consistency_across_lifecycle() {
    ...
    debug!("Verified working directory for active session: {:?}", active_session.working_directory);
    ...
    debug!("Verified working directory for inactive session: {:?}", inactive_session.working_directory);
    
    // Verify working directory is consistent
    assert_eq!(active_session.working_directory, inactive_session.working_directory);
