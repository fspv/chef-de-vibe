...
async fn test_session_id_mismatch_between_filename_and_content() {
    ...
    let mismatched_file = server.mock.projects_dir.join("filename-session-id.jsonl");
    fs::write(
        &mismatched_file,
...
{"uuid": "msg-1", "sessionId": "different-session-id", "type": "user", "message": {"role": "user", "content": "Test message"}}"#,
    ...
    assert_ne!(
        response.status(),
        200,
        "Should not return success for mismatched IDs"
    );
    ...
    if let Some(sessions) = body.get("sessions").and_then(|s| s.as_array()) {
        ...
        if has_filename_id {
            ...
            let session = sessions
                .iter()
                .find(|s| {
                    s.get("session_id").and_then(|id| id.as_str()) == Some("filename-session-id")
                })
                .unwrap();
...
async fn test_write_to_dead_claude_process() {
    ...
    let session_file_path = server.mock.projects_dir.join("dead-process-session.jsonl");
    ...
    let write_result = ws
        .send(Message::Text(
            r#"{"role": "user", "content": "Write to dead process"}"#.to_string(),
        ))
        .await;
    ...
    if write_result.is_ok() {
        ...
        assert!(
            close_received,
            "WebSocket should close after writing to dead process"
        );
    } else {
...
async fn test_malformed_json_from_claude_causes_websocket_closure() {
    ...
    let malformed_trigger =
        r#"{"control": "echo_malformed", "content": "{ broken json without closing"}"#;
...
async fn test_websocket_to_nonexistent_session() {
    ...
    if let Ok((mut ws, _)) = connect_result {
        ...
        assert!(
            ...
            "Connection should be refused for non-existent session"
        );
    }
}
...
async fn test_client_sends_invalid_json_via_websocket() {
    ...
    ws.send(Message::Text(
        r#"{"role": "user", "content": "Valid message"}"#.to_string(),
    ))
    .await
    .unwrap();
    ...
    let valid_after = ws
        .send(Message::Text(
            r#"{"role": "user", "content": "Valid after invalid"}"#.to_string(),
        ))
        .await;
...
async fn test_concurrent_writes_to_same_session() {
    ...
    // Create session
    let session_file_path = server.mock.projects_dir.join("concurrent-session.jsonl");
    let session_content = format!(
    ...
    let final_check = client
        .get(format!(
            "{}/api/v1/sessions/concurrent-session",
            server.base_url
        ))
        .send()
        .await
    ...
    assert_eq!(
        final_check.status(),
        200,
        "Session should still be accessible after concurrent writes"
    );
}
...
async fn test_session_file_corruption_during_active_session() {
    ...
    // Create session
    let session_file_path = server.mock.projects_dir.join("corruption-session.jsonl");
    let session_content = format!(
    ...
    if response.status() == 200 {
        ...
        ws.send(Message::Text(
            r#"{"role": "user", "content": "Before corruption"}"#.to_string(),
        ))
        .await
        .unwrap();
        ...
        let send_after_corruption = ws
            .send(Message::Text(
                r#"{"role": "user", "content": "After corruption"}"#.to_string(),
            ))
            .await;
        ...
        let get_response = client
            .get(format!(
                "{}/api/v1/sessions/corruption-session",
                server.base_url
            ))
            .send()
            .await
