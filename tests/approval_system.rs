...
// Helper functions
async fn connect_approval_websocket(ws_url: &str) -> Result<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>, Box<dyn std::error::Error>> {
    ...
    if let Ok(Message::Text(text)) = response {
        ...
        if parsed.get("id").is_some() && parsed.get("request").is_some() {
            ...
            if tool_name == expected_tool_name {
                ...
                return Err(format!("Expected tool_name '{}', got '{}'", expected_tool_name, tool_name).into());
            }
        } else {
            return Err(format!("Expected approval request with id and request fields, got message: {:?}", parsed).into());
        }
    }
...
async fn test_approval_websocket_basic_connection() {
    ...
    assert!(approval_ws_result.is_ok(), "Should be able to connect to approval WebSocket");
    ...
    match timeout_result {
        ...
        Ok(Some(Ok(Message::Text(text)))) => {
            ...
            if parsed["type"] == "pending_approvals" {
                ...
                assert_eq!(requests.len(), 0, "Should have no pending approvals initially");
            }
        }
        _ => {
...
async fn test_approval_websocket_connection_to_nonexistent_session() {
    ...
    let approval_ws_url = format!("{}/api/v1/sessions/nonexistent-session/claude_approvals_ws", server.ws_url);
...
async fn test_single_tool_approval_allow_flow() {
    ...
    match response {
        Message::Text(text) => {
            ...
            let first_content = content_array.first()
                .expect("Content array should not be empty");
            let text_content = first_content.get("text")
                .and_then(|v| v.as_str())
                .expect("First content item should have 'text' field");
            
            assert_eq!(
                text_content,
                "APPROVAL_TEST_SUCCESS_READ",
...
async fn test_debug_mock_claude_directly() {
    ...
    println!("Mock Claude stdout: {}", String::from_utf8_lossy(&output.stdout));
    println!("Mock Claude stderr: {}", String::from_utf8_lossy(&output.stderr));
...
async fn test_multiple_pending_approvals_accumulation() {
    ...
    assert!(request_data.get("id").is_some() && request_data.get("request").is_some(), "Should receive approval request with id and request fields");
    assert_eq!(request_data["request"]["tool_name"], "Read", "Should be Read tool request");
    ...
    let response2 = timeout(Duration::from_secs(3), approval_ws2.next()).await.unwrap().unwrap().unwrap();
    match response2 {
        Message::Text(text) => {
            let parsed: serde_json::Value = serde_json::from_str(&text).unwrap();
            ...
            if parsed.get("id").is_some() && parsed.get("request").is_some() {
                assert_eq!(parsed["request"]["tool_name"], "Read", "Should have request for Read tool");
                assert_eq!(parsed["id"], request_data["id"], "Should be the same request");
                println!("✓ Second client correctly received pending approval as individual message");
            } else {
...
async fn test_multiple_approval_clients_broadcast() {
    ...
    for (i, ws) in [&mut approval_ws1, &mut approval_ws2, &mut approval_ws3].iter_mut().enumerate() {
        let response = timeout(Duration::from_secs(3), ws.next()).await;
        match response {
            Ok(Some(Ok(Message::Text(text)))) => {
                println!("Client {} received: {}", i + 1, text);
                let parsed: serde_json::Value = serde_json::from_str(&text).unwrap();
                assert!(parsed.get("id").is_some() && parsed.get("request").is_some(), "Client {} should receive approval request with id and request fields", i + 1);
                assert_eq!(parsed["request"]["tool_name"], "Read", "Client {} should receive Read tool request", i + 1);
            ...
            Ok(Some(Ok(other))) => {
                panic!("Client {} received unexpected message type: {:?}", i + 1, other);
            }
            Ok(Some(Err(e))) => {
    ...
    assert_eq!(request_ids.len(), 3, "Should have received request_id from all 3 clients");
    assert_eq!(request_ids[0], request_ids[1], "Client 1 and 2 should receive same request_id");
    assert_eq!(request_ids[1], request_ids[2], "Client 2 and 3 should receive same request_id");
    println!("✓ All clients received the same approval request with request_id: {}", request_ids[0]);
