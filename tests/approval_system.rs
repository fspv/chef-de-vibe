...
async fn test_single_tool_approval_allow_flow() {
    ...
    // First consume the echoed control_request (due to new broadcast behavior)
    let echoed_msg = timeout(Duration::from_secs(2), main_ws.next())
        .await
        .expect("Should receive echoed message")
        .expect("WebSocket stream should not end")
        .expect("Should receive valid message");

    // Verify it's the echoed control_request
    if let Message::Text(text) = echoed_msg {
        let parsed: serde_json::Value = serde_json::from_str(&text).unwrap();
        assert_eq!(
    ...
    // Consume the echoed assistant message (due to new broadcast behavior)
    let echoed_assistant = timeout(Duration::from_secs(2), main_ws.next())
        .await
        .expect("Should receive echoed assistant message")
        .expect("WebSocket stream should not end")
        .expect("Should receive valid message");

    // Verify it's the echoed assistant message
    if let Message::Text(text) = echoed_assistant {
        let parsed: serde_json::Value = serde_json::from_str(&text).unwrap();
        assert_eq!(
            parsed.get("type").and_then(|v| v.as_str()),
            Some("assistant")
    ...
    // Now consume the automatic control_response generated by the system
    ...
    // Now should receive the processed assistant message from mock Claude
...
async fn test_single_tool_approval_deny_flow() {
    ...
    // Now consume the echoed assistant message (due to new broadcast behavior)
    let echoed_assistant = timeout(Duration::from_secs(2), main_ws.next())
        .await
        .expect("Should receive echoed assistant message")
        .expect("WebSocket stream should not end")
